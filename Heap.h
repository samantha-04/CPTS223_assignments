...
private:
	...
	{
		unsigned int child;
		T tmp = std::move(_items[hole]); //store the value of the hole
		
		for (; hole * 2 <= _items.size() - 1; hole = child) //while the hole has children
		{
			child = hole * 2;
		...
		_items[hole] = std::move(tmp); //put the value of the hole back into the heap
	}
	...
	{
		int hole = _items.size(); //hole is index of the last element in the heap
		_items.push_back(std::move(item)); //add new item to the end of the heap
		

		//percolate up
		for (; hole > 1 && item < _items[hole / 2]; hole /= 2) //while the item is less than its parent
		{
